...
/**
 * Shared "Verified" indicator, unifying what used to be two independent inline
 * implementations (ServiceCard.jsx's labeled badge and CareRequestDetail.jsx's
 * bare avatar-overlay checkmark). `isVerified` mirrors the backend's
 * `IsIdentityVerified` field — this is the only readiness dimension actually
 * exposed on caregiver browse/response DTOs today.
 *
 * variant="inline" (default): dot + "Verified" text + checkmark icon, for a
 *   name-adjacent row (ServiceCard-style layout).
 * variant="overlay": compact icon-only checkmark, for placement on top of an
 *   avatar (CareRequestDetail-style layout, no room for the full badge).
 */
const VerifiedBadge = ({ isVerified, variant = 'inline' }) => {
  if (!isVerified) return null;

  if (variant === 'overlay') {
    return (
      <span className="verified-badge-overlay" title="Verified">✓</span>
    );
  }
