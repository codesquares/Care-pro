...
class MatchingService {
  ...
  static async fetchAvailableCaregivers() {
    ...
    if (!token) {
      throw new Error('No authentication token found for API calls');
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`API returned ${response.status} when fetching caregivers`);
      }

      const data = await response.json();

      if (!data || !Array.isArray(data)) {
        throw new Error('Invalid data format received from caregivers API');
      }
